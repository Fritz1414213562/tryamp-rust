...
use super::constant::{DEFAULT_BUFFER_SCALE, COORDINATE_BLOCK_OFFSET, COORDINATE_BLOCK_SCALE};
...
impl DCDParser {
	...
	//	let mut file: BufReader<R> = BufReader::<R>::with_capacity(1_000_000_000, file);
		...
		let coord_block_size: usize = COORDINATE_BLOCK_SCALE * (atom_num as usize * std::mem::size_of::<i32>() + COORDINATE_BLOCK_OFFSET);
		...
		loop {
			match file.read(&mut buffer) {
				Ok(0) => break,
				Ok(buffer_size) => {
					match buffer[..buffer_size].chunks(coord_block_size).map(|mut xyz_bytes| self.read_xyz(&mut xyz_bytes)).collect::<Result<Vec<Coordinates<f32>>, String>>() {
	...
	fn read_xyz<R: Read>(&self, bytes: &mut R) -> Result<Coordinates<f32>, String> {
